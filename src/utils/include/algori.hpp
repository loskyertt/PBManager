...
struct AVLNode {
  ...
  AVLNode(const Key &k, const Value &v) : key(k), data(v), height(1), left(NULL), right(NULL) {}
};
...
class AVLTree {
 ...
 private:
  ...
  std::vector<Value> _M_findValues(AVLNode<Key, Value> *node,
      const Key &key,
 ...
 public:
  ...
  void setFuzzyFunc(const std::function<bool(const Key &k1, const Key &k2)> ptr) {
...
template <typename Key, typename Value>
std::vector<Value> AVLTree<Key, Value>::_M_findValues(AVLNode<Key, Value> *node,
    const Key &key,
    std::vector<Value> &matches,
...
template <typename Key, typename Value>
AVLNode<Key, Value> *AVLTree<Key, Value>::_M_find(AVLNode<Key, Value> *node, const Key &key) {
  if (node == nullptr) {
    return nullptr;
  }
...
template <typename Key, typename Value>
AVLNode<Key, Value> *AVLTree<Key, Value>::insert(AVLNode<Key, Value> *node, const Key &key, const Value &value) {
  if (!node) {
...
template <typename Key, typename Value>
AVLNode<Key, Value> *AVLTree<Key, Value>::deleteNode(AVLNode<Key, Value> *root, const Key &key) {
  if (!root) {
    return root;
  }
...
template <typename Key, typename Value>
Value &AVLTree<Key, Value>::findValue(AVLNode<Key, Value> *node, const Key &key) {
  if (!node) {
    throw "not found";
  }
