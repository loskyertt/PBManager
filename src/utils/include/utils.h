...
struct TelephoneItem {
  ...
  TelephoneItem(const string &name,
      const string &city,
      const string &p,
      const string &p1,
      const string &p2,
  ...
  friend bool operator==(const TelephoneItem &item1, const TelephoneItem &item2);

  friend std::ostream &operator<<(std::ostream &os, const TelephoneItem &item);

  friend std::ostream &operator<<(std::ostream &os, const std::vector<TelephoneItem> &items);
};

typedef std::function<bool(const std::string &k1, const std::string &k2)> cmp_ptr;
...
#define TELEPHONES_FIND_API(FUNC_NAME, PARAMS_NAME)                                         \
  inline std::vector<TelephoneItem> findItemBy##FUNC_NAME(const std::string &PARAMS_NAME) { \
    return PARAMS_NAME##Map[PARAMS_NAME];                                                   \
  }

#define TELEPHONE_FUZZY_API(FUNC_NAME, PARAMS_NAME)                                                            \
  inline std::vector<TelephoneItem> findFuzzy##FUNC_NAME(const std::string &PARAMS_NAME, const cmp_ptr &ptr) { \
...
class TelephoneDirect {
 ...
 public:
  ...
  AVLTree<std::string, std::vector<TelephoneItem>> ophoneMap, companyMap, addressMap, belong_groupMap, cityMap;
 ...
 public:
  ...
  inline std::vector<TelephoneItem> findFuzzyOfficePhone(const std::string &ophone, const cmp_ptr &ptr) {
