...
void TelephoneDirect::load(const string &path) {
  ...
  while (in_file.getline(buff, MAX_BUFF_SIZE)) {
    s_time++;
    sscanf(buff, "%s %s %s %s %s %s %s %s %[^\n]\n", name, city, phone, hphone, ophone, email, company, addr, time);
    TelephoneItem item = TelephoneItem(string(name),
        string(city),
        string(phone),
        string(hphone),
        string(ophone),
...
#define ERASE_FROM_ARRAY_MAP(obj)                                                                                    \
  if (obj##Map.find(item.obj)) {                                                                                     \
    ...
    arr.erase(std::remove_if(arr.begin(), arr.end(), [&](const TelephoneItem &in_item) { return item == in_item; }), \
        arr.end());                                                                                                  \
  }
...
bool operator==(const TelephoneItem &lhs, const TelephoneItem &rhs) {
  return lhs.name == rhs.name && lhs.city == rhs.city && lhs.phone == rhs.phone && lhs.hphone == rhs.hphone &&
      lhs.ophone == rhs.ophone && lhs.email == rhs.email && lhs.company == rhs.company && lhs.address == rhs.address &&
      lhs.belong_group == rhs.belong_group && lhs.brief == rhs.brief && lhs.add_time == rhs.add_time;
}

std::ostream &operator<<(std::ostream &os, const TelephoneItem &item) {
  os << "姓名: " << item.name << " 城市: " << item.city << " 手机: " << item.phone << " 住宅电话: " << item.hphone
     << " 办公电话: " << item.ophone << " 电子邮件: " << item.email << " 公司: " << item.company
     << " 地址: " << item.address;
...
std::ostream &operator<<(std::ostream &os, const std::vector<TelephoneItem> &items) {
  for (auto item : items) {
    os << item;
  }
  return os;
